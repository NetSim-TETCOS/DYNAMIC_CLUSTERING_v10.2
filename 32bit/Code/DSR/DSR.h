...
#define _DYNAMIC_CLUSTERING	
#define CLUSTER_INTERVAL 5*SECOND

/*****************Uncomment this to run Dynamic Clustering instead of DSR **********************
If Dynamic Clustering is enabled in WSN, instead of running DSR clustering is used. The Dynamic Clustering 
Codes are  available in Dynamic_Clustering.c. Please read that file for implementation specifics
Cluster heads are dynamically selected for each cluster based on sensor energy
and static routes can be defined between them
**************************************END*****************************************/
...
//double **fn_netsim_matlab_run(int s_count, int n_cls, **POWER);
double fn_netsim_matlab_finish();
static int *cluster_head, *cluster_id, *cluster_size;
