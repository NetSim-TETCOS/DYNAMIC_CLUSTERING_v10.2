...
int fn_NetSim_dynamic_clustering_run()
{
	...
	fn_netsim_matlab_run(sensor_count, NUMBEROFCLUSTERS, pstruDevicePower);
...
void fn_NetSim_Dynamic_Clustering_Init()
{
	...
	cluster_head = (int*)calloc(NUMBEROFCLUSTERS, sizeof*(cluster_head));//To store the cluster head id's of each cluster
	cluster_id = (int*)calloc(sensor_count, sizeof*(cluster_id));//To store the cluster id of each sensor
	cluster_size = (int*)calloc(sensor_count, sizeof*(cluster_size));//To store size of each cluster
