	...
	case PHYSICAL_OUT_EVENT:
		{
			...
			double dDataRate;
			bool txFlag=false;
			//Updating Dynamic Clustering/Data Metric
			if(pstruEventDetails->nDeviceId == pstruEventDetails->pPacket->nSourceId && pstruEventDetails->pPacket->nPacketType == PacketType_Custom)
				Data_Packet_Sent_PL[pstruEventDetails->nDeviceId-1] += 1;
	...
	case PHYSICAL_IN_EVENT: 
		{
			...
			else
			{
				...
				//Add mac in event
				fnpAddEvent(pstruEventDetails);
				//Update Dynamic Clustering/Data metrics
				if(pstruEventDetails->nDeviceId == get_first_dest_from_packet(pstruEventDetails->pPacket) && pstruEventDetails->pPacket->nPacketType == PacketType_Custom)
					Data_Packet_Received_PL[pstruEventDetails->nDeviceId-1] += 1;
...
{
	PMETRICSNODE table = init_metrics_node(MetricsNode_Table, "DATA Metrics", NULL);
	...
	add_table_heading(table, "Lifetime(s)", true, 0);
	int i;
	for (i = 0; i < nDeviceCount; i++)
	{
		add_table_row_formatted(false, table, "%d,%d,%d,%f,", 
			NETWORK->ppstruDeviceList[i]->nDeviceId,
	...
	PMETRICSNODE menu = init_metrics_node(MetricsNode_Menu, "DATA Metrics", NULL);
