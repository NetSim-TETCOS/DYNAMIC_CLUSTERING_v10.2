...
#define MAINLINE 2
	...
	#ifndef _DYNAMIC_CLUSTERING
	_declspec(dllexport) POWER** pstruDevicePower;

	////For Dynamic Clustering/Data Metrics
	//// Array size is 1001 and can support upto 1000 sensors and 1 sink
	_declspec(dllexport) int Data_Packet_Sent_NWL[1001] ;
	_declspec(dllexport) int Data_Packet_Received_NWL[1001] ;
	_declspec(dllexport) int Data_Packet_Sent_PL[1001] ;
	_declspec(dllexport) int Data_Packet_Received_PL[1001] ;
	_declspec(dllexport) double Lifetime[1001];
	...
	_declspec(dllimport) int Data_Packet_Sent_NWL[1001] ;
	_declspec(dllimport) int Data_Packet_Received_NWL[1001] ;
	_declspec(dllimport) int Data_Packet_Sent_PL[1001] ;
	_declspec(dllimport) int Data_Packet_Received_PL[1001] ;
	_declspec(dllimport) double Lifetime[1001];
#endif
